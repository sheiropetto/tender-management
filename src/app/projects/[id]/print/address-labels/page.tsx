...
export default function AddressLabelsPage() {
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <h1 className="text-xl font-medium text-zinc-900 mb-1">Submission Address Labels</h1>
      <p className="text-sm text-zinc-400 mb-6">{project.shortName || project.name}</p>

      <div className="rounded-xl border border-zinc-200 bg-white p-5">
        <h3 className="text-sm font-medium text-zinc-800 mb-3">Submission Address</h3>
        {project.submissionAddress ? (
          <pre className="text-sm text-zinc-600 whitespace-pre-wrap mb-4 font-sans">
            {project.submissionAddress}
          </pre>
        ) : (
          <p className="text-sm text-zinc-400 mb-4 italic">No submission address set for this project.</p>
        )}
        <div className="flex gap-3">
          <Link
            href={`/projects/${project.id}/print/address-labels/preview?type=original`}
          ...
          <Link
            href={`/projects/${project.id}/print/address-labels/preview?type=copy`}
            className="flex-1 rounded-lg border border-zinc-200 bg-white px-4 py-2.5 text-sm font-medium text-zinc-600 hover:bg-zinc-50 transition-colors text-center"
          >
            Print COPY
          </Link>
